...
- 直接启动后端 API：`.venv/bin/uvicorn app.main:app --reload`（首次启动会自动创建 `data/` 目录）
...
- 数据存储在 `data/` 目录下的 JSON 文件中（`creators.json`、`tags.json`、`videos.json` 等），纳入 git 版本管理。模型使用 Pydantic v2 BaseModel，通过 `app/store/repo.py` 的 `JsonRepo[T]` 泛型仓库进行读写。
...
- 所有个人管理数据都必须本地化：观看状态、标签关系、同步日志都以 JSON 文件存储在 `data/` 目录下，纳入 git 版本管理，不依赖 B 站账号状态。

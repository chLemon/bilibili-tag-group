...
5. `_wait_for_cards` 保险等卡片，提取当前页所有卡片（单张卡片解析失败仅跳过）；按 bvid 全局去重（`seen_bvids`），同一 bvid 即便多次出现也只入列一次，防御 SPA 翻页后 DOM 未刷新时把上一页卡片重复 append
6. 若当前页 bvid 与 `known_videos` 有交集则提前结束；否则翻页前随机延迟 2.0–4.0s 继续

`known_videos` 由调用方（`sync_creator`）传入本地已存的该 UP 主视频集合。早停只省抓取量——返回前会把 `known_videos` 中本次未抓到的视频补回，返回值始终是"已知 ∪ 本次新抓"的并集，不因早停而缺失。

## 风控与跳页重试

B 站 SPA 跳页后，前端发 XHR 请求目标页 cards 数据；触发安全风控时 B 站返回 412 错误页，SPA 收到错误后**只更新激活页码 UI，不刷新 cards 列表**——DOM 里仍是上一页的卡片。表现：`_wait_for_new_bvid` 等到 30s 超时仍不见新 bvid，诊断时 cards 数与上一页一致、unique bvid 不变、激活页码已变。
